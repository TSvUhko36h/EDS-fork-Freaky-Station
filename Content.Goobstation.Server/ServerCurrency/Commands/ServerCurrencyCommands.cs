...
namespace Content.Goobstation.Server.ServerCurrency.Commands
{
    ...
    public sealed class BalanceServerCurrencyCommand : IConsoleCommand
    {
        ...
        public void Execute(IConsoleShell shell, string argStr, string[] args)
        {
            ...
            var balance = Loc.GetString("server-currency-balance-command-return",
                ("balance", _currencyMan.Stringify(_currencyMan.GetBalance(player.UserId))));

            _chatManager.ChatMessageToOne(ChatChannel.Local, balance, balance, EntityUid.Invalid, false, player.Channel);
    ...
    public sealed class GiftServerCurrencyCommand : IConsoleCommand
    {
        ...
        public void Execute(IConsoleShell shell, string argStr, string[] args)
        {
            ...
            else if (targetPlayer == player.UserId)
            {
            ...
            if (!_currencyMan.CanAfford(player.UserId, amount, out var balance))
            {
                shell.WriteError(Loc.GetString("server-currency-gift-command-error-2", ("balance", balance)));
                return;
            }

            _currencyMan.TransferCurrency(player.UserId, targetPlayer, amount);

            var giver = Loc.GetString("server-currency-gift-command-giver", ("player", args[0]), ("amount", _currencyMan.Stringify(amount)));
            var receiver = Loc.GetString("server-currency-gift-command-reciever", ("player", player.Name), ("amount", _currencyMan.Stringify(amount)));

            if (plyMgr.TryGetSessionById(targetPlayer, out var targetPlayerSession))
                _chatManager.ChatMessageToOne(ChatChannel.Local, receiver, receiver, EntityUid.Invalid, false, targetPlayerSession.Channel);
            _chatManager.ChatMessageToOne(ChatChannel.Local, giver, giver, EntityUid.Invalid, false, player.Channel);
    ...
    public sealed class AddServerCurrencyCommand : IConsoleCommand
    {
        ...
        public string Command => "balance:add";
    ...
    public sealed class RemoveServerCurrencyCommand : IConsoleCommand
    {
        ...
        public string Command => "balance:rem";
    ...
    public sealed class SetServerCurrencyCommand : IConsoleCommand
    {
        ...
        public string Command => "balance:set";
    ...
    public sealed class GetServerCurrencyCommand : IConsoleCommand
    {
        ...
        public string Command => "balance:get";
